...
**demonstration GUI** that presents all three in one place and lets a reader drive the harness
without a terminal. It does not produce a solver.
...
| # | Requirement | Type |
| --- | --- | :-: |
| 10.1.2 | **No secondary implementation.** Production logic is written once; no reference implementation, oracle, or "obvious slow way" may be written to check another. Writing a computation twice yields two things to keep correct and no way to tell which one is wrong. | NF |
...
| 10.3.3 | The figure generator for explainer assets has **no third-party dependency** and runs on a system Python, because the figures are committed artifacts that must stay regenerable years from now on any machine with a Python on it. | NF |
...
| 10.3.9 | **Every tool a documented command needs is a declared dependency**, in a group that command's instructions name. A documented command that cannot run after the documented install is a defect of the packaging, not of the reader. Under 10.3.1's split this binds hardest on the commands that need the harness: `generate`, `run` and the catalog build name the group that carries it, and a command invoked without that group **fails saying which group supplies it** rather than raising an import error naming a package the reader never asked for. | NF |
...
| 10.3.11 | **A committed generated artifact is current with respect to its generator.** Editing a source or a generator without rerunning it leaves the repository shipping the older output, and the tests read the committed copy, which is self-consistent and wrong. Currency is asserted by running the shipped generator and comparing, never by a second implementation of it (10.1.2). | NF |
...
| 11.9 | A reference or oracle implementation of the objective | 10.1.2. The cost is accepted: the anchors constrain the extremes and the invariants, so no test cross-checks the objective on an arbitrary interior layout |
