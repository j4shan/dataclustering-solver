...
One page, served locally, carrying three sections: the problem, a benchmark of assignment
strategies against the harness, and the production design. One is displayed at a time and the
others wait behind the navigation, so the reader chooses where to be rather than scrolling past
what they did not ask for. Each section is split left and right — an explanatory pane beside a
demonstrative one — and the fixed order they are presented in is the order the argument runs:
understand, then try, then build.
...
| # | Requirement | Type |
| --- | --- | :-: |
| 12.1.1 | A **single page** with three sections in fixed order: A problem statement, B assignment strategy benchmark, C production design. Each is a left/right pair on a wide viewport. **One section is displayed at a time**, chosen by 12.1.3's control; the other two are not laid out. The order is what the control presents and what the letters name, not a sequence the page is read through in one pass. | F |
...
| 12.1.9 | Type is set for **sustained reading**: a bounded measure on prose, leading generous enough for long passages, and numerals aligned in tables. Section A's formulation pane is the case this requirement is written for. | NF |
...
| 12.2.8 | **One command launches the page, and survives being typed twice.** Before binding it probes the target host and port, and an instance already serving is **reported rather than collided with** — an `Address already in use` traceback is the failure most likely to happen in front of an audience, in the least readable form there is. On finding one it offers **restart, shutdown, or leave it running**, and leaving it running is what pressing Enter does. A listener that is **not this server is never terminated**: the port is this project's guess, and the process holding it is not this command's to stop. | F |
| 12.2.8.1 | A launch that binds **opens the page in the reader's browser**, adding no dependency to do it (10.3.1) and fetching nothing (12.2.2). Suppressed by an explicit opt-out, and whenever stdin is not a terminal — a test run and a CI job must never spawn one. | NF |
...
| 12.3.5 | **Left**: a **closed, ordered editorial walkthrough** with three prose stages and the static illustrations specified by 12.3.5.3. It begins entirely inside a closet analogy, repeats the same scenario as a data-warehouse fetch, and ends by handing the reader to the detailed framework on the right. Nothing else appears in the pane — no outbound link list, event table, index table, or alternate scenario. | F |
| 12.3.5.1 | The left pane carries the three narrative requirements 12.3.5.1.1–12.3.5.1.3 in that order. Connecting prose may bridge them, but may not introduce a fourth stage or move the technical mapping ahead of the closet-only account. | F |
| 12.3.5.1.1 | Picture a closet analogy to layout the key concepts: drawer - storage container, closet - data warehouse, wasted effort - records materialized not selected, without mentioning any database term. Conclude with a question to bring forward "a smart organizer". | F |
| 12.3.5.1.2 | Walkthrough the same scenario in a data warehouse fetch process, map key concepts to technical terms. Emphasize the shared challenge and the opportunity for improvement. | F |
| 12.3.5.1.3 | Inform the reader that the formal problem statement is presented in the right panel. | F |
| 12.3.5.2 | Every illustration in the pane carries a **visible title**, not only an accessible one. The titles are *One outfit, one whole drawer* for `A.closet-problem`, *The same fetch in a warehouse* for `A.fetch-process`, and *What a smart organizer can change* for `A.smart-organizer`. | F |
| 12.3.5.3 | The pane inserts exactly the three illustration prompts below at the stated narrative locations. All three reuse one composition, object set, request, and colour assignment so the reader sees one scenario acquire technical names and then one controlled change; a new drawing may change only what its prompt names. | F |

**12.3.5.3 illustration prompts**

- **After (1), before (2) — `A.closet-problem`, *One outfit, one whole drawer*.** Show one morning's outfit request beside a closet with an inventory list on its door. The list points to one drawer; the whole drawer has been pulled out. Separate the wanted garment from the other garments that were lifted and must be put back, making the wasted effort visibly larger than the useful effort. End the accompanying prose with the **smart organizer** question. The figure and its surrounding stage contain no database vocabulary, symbols, formulas, code, or technical mapping labels.
- **After (2) — `A.fetch-process`, *The same fetch in a warehouse*.** Repeat the first figure's geometry and object relationships as one left-to-right fetch: query request → **index table** lookup → storage-container activation → records materialized → records selected and records materialized but not selected. Map closet to data warehouse, drawer to storage container, garment to record, morning's outfit request to query, inventory list to **index table**, and wasted effort to records materialized but not selected. Directional arrows establish process order; selected and unselected materialized records differ by both label and treatment, never colour alone. The accompanying prose states that both scenarios pay for the whole containing unit and identifies changing record assignment as the opportunity for improvement.
- **Between (2) and (3) — `A.smart-organizer`, *What a smart organizer can change*.** Put two versions of the same warehouse fetch side by side. Keep the query, records, index-table result, container capacity, and selected records fixed; change only which records share storage containers. Show the second assignment materializing fewer unselected records. Present this as the tunable opportunity that the right panel formalizes, not as a ranked or recommended assignment strategy.

| # | Requirement | Type |
| --- | --- | :-: |
| 12.3.5.4 | The drawings are flat editorial diagrams — not photoreal, 3D, screenshots, UI chrome, server racks, or spreadsheet grids. They use the warm paper ground and ink of 12.1.5 / 12.7, page sans for labels, one restrained repeating palette, and single-stroke directional arrows. The first figure uses only closet objects and ordinary-language labels; the second and third may add technical labels after 12.3.5.1.1 has concluded. | F |

| # | Requirement | Type |
| --- | --- | :-: |
| 12.3.6 | Vocabulary is introduced in two passes. Stage (1) uses closet, drawer, garment, inventory list, morning, outfit, handling, putting back, wasted effort, and **smart organizer**; it contains no database term, including data, warehouse, storage, container, record, row, query, index, materialized, selected, layout, assignment, or strategy. Stage (2) explicitly maps the same nouns and actions to data warehouse, storage container, record, query, **index table**, materialized, selected, layout, and assignment strategy. Stage (3) may use those technical terms only to identify what the right panel develops. | F |
| 12.3.6.1 | The pane is written in **plain language**. The prohibition in 12.3.5.1.1 applies to that stage's reader-facing copy and illustration, not to the requirement's authorial mapping. Stage (2) defines every technical term at the moment it replaces a closet object or action; no glossary precedes the walkthrough. Serves 12.3.6. | F |
...
| 13.5 | Editing the dataset, the selection log, or the layout from the GUI | 4.10 and 5.11 — datasets are immutable and the selection table is observed history. The GUI reads and scores; it never writes a dataset |
...
| 13.15 | Background or daemon mode, a process supervisor, or stopping an instance running on another machine | 12.2.10 — the process model is one foreground command ended with Ctrl-C, and 12.2.8's probe removes the reason a demonstration would reach for a background one: a second invocation now finds the first rather than failing against it. Detaching would put the server's own output somewhere a reader has to go looking for. Supervision of the deployed instance belongs to the platform running the image (10.3.12), not to this command |
...
| 13.17 | Outbound links from Section A's illustration pane to the interactive canvas (9.12) and the generated figures (9.7) | 12.3.5 — the pane is the closed six-beat closet sequence. Those artifacts remain in the repository and stay reachable from the README; they are not a footer of this pane |
