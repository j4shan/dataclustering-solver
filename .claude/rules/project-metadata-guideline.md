...
`site/` is the published exhibit. It is served as-is: whatever sits in it reaches the
public, and nothing else does. The directory layout is the URL layout, so a path that
works on disk works in the browser.
