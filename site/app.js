...
// The published copy arrives with every fragment already in it (12.2.14), so there is
// nothing to mount and the loop above does nothing.  Zoom still has to reach the figures
// those fragments drew, and `attachZoom` is idempotent, so one pass over the whole
// document covers both the served page and the published one.
attachZoom(document.body);
selectSection();
