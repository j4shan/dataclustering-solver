#!/usr/bin/env python3
"""Render the documentation figures from the shared closet scenario.

Reads docs/data/closet-scenario.json and writes light/dark SVG pairs into
docs/img/. Run after editing the scenario:
...
SCENARIO = ROOT / "docs" / "data" / "closet-scenario.json"
...
def selections(scenario):
    """query_id -> set of event ids selected by it (the usage log, pivoted)."""
    sel = {q["id"]: set() for q in scenario["queries"]}
    for event_id, query_id in scenario["usage_log"]:
        sel[query_id].add(event_id)
    return sel


def evaluate(scenario, layout):
    """Activation, waste and cost for one layout, per §4-§5 of the formulation."""
...
def figure_matrix(scenario, theme):
    ...
    return svg(width, height, "".join(body), c,
               "Usage matrix under two layouts, showing container activation and waste")
...
def figure_zoom(scenario, theme):
    ...
    for ci, (tag, opened) in enumerate([(spec["activated_tag"], True),
        ...
        body.append(text(cx + 16, card_top + 56,
                         "summary page says:  " + " · ".join(con["labels"]),
                         fill=c["muted"], size=10.5))
    ...
    body.append(text(margin, fy + 17,
                     "Right: the summary page rules the container out before any read, "
...
def main():
    ...
    for theme in THEMES:
        (OUTDIR / f"usage-matrix.{theme}.svg").write_text(
