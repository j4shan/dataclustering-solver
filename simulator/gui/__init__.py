...
**The repository tree is the site tree.**  `site/` is what this server hands over, so the
directory layout on disk is the URL layout in the browser and a path that resolves for a
reader of the repository resolves for a reader of the page.  There is one root and no
mapping: what is not in `site/` cannot be requested, which is what keeps diagram sources
and figure prompts — `resources/`, never served — out of a public deployment by
construction rather than by an exclude list someone has to maintain.
...
#: The served tree: the page, its assets, and its figures.
